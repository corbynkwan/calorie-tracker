...
export default function FoodTable({id}) {
  ...
  const rows = useSelector(state => state.foods.restaurantFoods);
  ...
  const onAdd = () => {
    selected.map((selectedName) => {
      const foodToAdd = rows.filter((food) => food.name === selectedName);
      const {
        name,
        calories,
        fat,
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar onAdd={onAdd} numSelected={selected.length} />
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
